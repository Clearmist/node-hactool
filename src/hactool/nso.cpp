...
static void *nso_uncompress(nso0_ctx_t *ctx) {
    ...
    nso0_header_t *new_nso = static_cast<nso0_header_t *>(calloc(1, size));
    if (new_nso == NULL) {
...
void nso0_process(nso0_ctx_t *ctx) {
    ...
    ctx->header = static_cast<nso0_header_t *>(malloc(size));
    if (ctx->header == NULL) {
    ...
    ctx->uncompressed_header = static_cast<nso0_header_t *>(nso_uncompress(ctx));
