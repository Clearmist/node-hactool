...
void nca0_romfs_process(nca0_romfs_ctx_t *ctx) {
    ...
    if ((ctx->tool_ctx->action & (ACTION_EXTRACT | ACTION_LISTROMFS)) && ctx->header.header_size == NCA0_ROMFS_HEADER_SIZE) {
        ...
        ctx->directories = static_cast<romfs_direntry_t *>(calloc(1, ctx->header.dir_meta_table_size));
        ...
        ctx->files = static_cast<romfs_fentry_t *>(calloc(1, ctx->header.file_meta_table_size));
        if (ctx->files == NULL) {
