...
void settings_add_titlekey(hactool_settings_t *settings, const unsigned char *rights_id, const unsigned char *titlekey) {
    ...
    if (settings->known_titlekeys.count == 0) {
        settings->known_titlekeys.titlekeys = static_cast<titlekey_entry_t *>(malloc(1 * sizeof(titlekey_entry_t)));
    } else if ((settings->known_titlekeys.count & (settings->known_titlekeys.count + 1)) == 0) {
        settings->known_titlekeys.titlekeys = static_cast<titlekey_entry_t *>(realloc(settings->known_titlekeys.titlekeys, 2 * (settings->known_titlekeys.count + 1) * sizeof(titlekey_entry_t)));
    }
