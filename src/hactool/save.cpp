...
void save_duplex_storage_init(duplex_storage_ctx_t *ctx, duplex_fs_layer_info_t *layer, void *bitmap, uint64_t bitmap_size) {
    ...
    ctx->bitmap.bitmap = static_cast<uint8_t *>(malloc(bitmap_size >> 3));
...
remap_segment_ctx_t *save_remap_init_segments(remap_header_t *header, remap_entry_ctx_t *map_entries, uint32_t num_map_entries) {
    remap_segment_ctx_t *segments = static_cast<remap_segment_ctx_t *>(calloc(1, sizeof(remap_segment_ctx_t) * header->map_segment_count));
    unsigned int entry_idx = 0;

    for (unsigned int i = 0; i < header->map_segment_count; i++) {
        remap_segment_ctx_t *seg = &segments[i];
        seg->entry_count = 0;
        remap_entry_ctx_t **ptr = static_cast<remap_entry_ctx_t **>(malloc(sizeof(remap_entry_ctx_t *) * (seg->entry_count + 1)));
        if (!ptr) {
        ...
        while (entry_idx < num_map_entries && map_entries[entry_idx - 1].virtual_offset_end == map_entries[entry_idx].virtual_offset) {
            ...
            ptr = static_cast<remap_entry_ctx_t **>(realloc(seg->entries, sizeof(remap_entry_ctx_t *) * (seg->entry_count + 1)));
            if (!ptr) {
...
void save_ivfc_storage_init(hierarchical_integrity_verification_storage_ctx_t *ctx, uint64_t master_hash_offset, ivfc_save_hdr_t *ivfc) {
    ...
    integrity_verification_info_ctx_t* init_info = (integrity_verification_info_ctx_t*) malloc(2000);
    ...
    ctx->level_validities = static_cast<validity_t **>(malloc(sizeof(validity_t *) * (ivfc->num_levels - 1)));
    for (unsigned int i = 1; i < ivfc->num_levels; i++) {
        ...
        level_data->block_validities = static_cast<validity_t *>(calloc(1, sizeof(validity_t) * level_data->sector_count));
...
void save_ivfc_storage_read(integrity_verification_storage_ctx_t *ctx, void *buffer, uint64_t offset, size_t count, int32_t verify) {
    ...
    uint8_t hash[0x20] = {0};
    uint8_t *data_buffer = static_cast<uint8_t *>(calloc(1, ctx->sector_size));
...
validity_t save_ivfc_validate(hierarchical_integrity_verification_storage_ctx_t *ctx, ivfc_save_hdr_t *ivfc) {
    ...
    for (unsigned int i = 0; i < ivfc->num_levels - 1 && result != VALIDITY_INVALID; i++) {
        ...
        uint8_t *buffer = static_cast<uint8_t *>(malloc(block_size));
...
void save_process(save_ctx_t *ctx) {
    ...
    ctx->data_remap_storage.map_entries = static_cast<remap_entry_ctx_t *>(calloc(1, sizeof(remap_entry_ctx_t) * ctx->data_remap_storage.header->map_entry_count));
    ...
    ctx->duplex_layers[1].data_a = static_cast<uint8_t *>(malloc(ctx->header.layout.duplex_l1_size));
    ...
    ctx->duplex_layers[1].data_b = static_cast<uint8_t *>(malloc(ctx->header.layout.duplex_l1_size));
    ...
    ctx->duplex_layers[2].data_a = static_cast<uint8_t *>(malloc(ctx->header.layout.duplex_data_size));
    ...
    ctx->duplex_layers[2].data_b = static_cast<uint8_t *>(malloc(ctx->header.layout.duplex_data_size));
    ...
    bitmap = static_cast<uint8_t *>(malloc(ctx->duplex_storage.layers[0]._length));
    ...
    ctx->meta_remap_storage.map_entries = static_cast<remap_entry_ctx_t *>(malloc(sizeof(remap_entry_ctx_t) * ctx->meta_remap_storage.header->map_entry_count));
   ...
   /* Initialize journal map. */
    ctx->journal_map_info.map_storage = static_cast<uint8_t *>(malloc(ctx->header.layout.journal_map_table_size));
    ...
    ctx->journal_storage.map.entries = static_cast<journal_map_entry_t *>(malloc(sizeof(journal_map_entry_t) * ctx->journal_storage.map.header->main_data_block_count));
    ...
    if (ctx->header.layout.version < 0x50000) {
        ctx->fat_storage = static_cast<uint8_t *>(malloc(ctx->header.layout.fat_size));
        ...
        ctx->fat_storage = static_cast<uint8_t *>(malloc(ctx->fat_ivfc_storage._length));
