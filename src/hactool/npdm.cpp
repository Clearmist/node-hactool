    ...
    for (uint32_t i = 0; i < num_descriptors; i++) {
        ...
        switch (low_bits) {
            ...
            case 6: /* Map IO/Normal. */
                cur_mmio = static_cast<kac_mmio_t *>(calloc(1, sizeof(kac_mmio_t)));
            ...
            case 7: /* Map Normal Page. */
                page_mmio = static_cast<kac_mmio_t *>(calloc(1, sizeof(kac_mmio_t)));
                if (page_mmio == NULL) {
                ...
                break;
            case 11: /* IRQ Pair. */
                cur_irq = static_cast<kac_irq_t *>(calloc(1, sizeof(kac_irq_t)));
                if (cur_irq == NULL) {
...
static void sac_parse(char *sac, uint32_t sac_size, sac_entry_t *r_host, sac_entry_t *r_accesses, sac_entry_t **out_hosts, sac_entry_t **out_accesses) {
    ...
    while (ofs < sac_size) {
        ...
        cur_entry = static_cast<sac_entry_t *>(calloc(1, sizeof(sac_entry_t)));
        if (ctrl & 0x80) {
