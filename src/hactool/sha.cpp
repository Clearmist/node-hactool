...
sha_ctx_t *new_sha_ctx(hash_type_t type, int hmac) {
    ...
    ctx = static_cast<sha_ctx_t *>(malloc(sizeof(*ctx)));
    ...
    if (mbedtls_md_setup(&ctx->digest, mbedtls_md_info_from_type(static_cast<mbedtls_md_type_t>(type)), hmac)) {
...
void sha_update(sha_ctx_t *ctx, const void *data, size_t l) {
    mbedtls_md_update(&ctx->digest, static_cast<const unsigned char *>(data), l);
}
...
void sha256_get_buffer_hmac(void *digest, const void *secret, size_t s_l, const void *data, size_t d_l) {
    sha_ctx_t *ctx = static_cast<sha_ctx_t *>(malloc(sizeof(*ctx)));
    ...
    if (mbedtls_md_setup(&ctx->digest, mbedtls_md_info_from_type(static_cast<mbedtls_md_type_t>(HASH_TYPE_SHA256)), 1)) {
        FATAL_ERROR("Failed to set up hash context!");
    }

    if (mbedtls_md_hmac_starts(&ctx->digest, static_cast<const unsigned char *>(secret), s_l)) {
        FATAL_ERROR("Failed to set up HMAC secret context!");
    }

    if (mbedtls_md_hmac_update(&ctx->digest, static_cast<const unsigned char *>(data), d_l)) {
        FATAL_ERROR("Failed processing HMAC input!");
    }

    if (mbedtls_md_hmac_finish(&ctx->digest, static_cast<unsigned char *>(digest))) {
