...
void ini1_process(ini1_ctx_t *ctx) {
    ...
    ctx->header = static_cast<ini1_header_t *>(malloc(raw_header.size));
    if (ctx->header == NULL) {
...
static void *kip1_uncompress(kip1_ctx_t *ctx, uint64_t *size) {
    ...
    *size = kip1_get_size_from_header(&new_header);
    unsigned char *new_kip = static_cast<unsigned char *>(calloc(1, *size));
...
void kip1_process(kip1_ctx_t *ctx) {
    ...
    ctx->header = static_cast<kip1_header_t *>(malloc(size));
