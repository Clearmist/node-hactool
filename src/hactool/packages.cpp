...
void pk11_process(pk11_ctx_t *ctx) {
    ...
    if (ctx->is_mariko) {
        ...
        ctx->mariko_bl = static_cast<uint8_t *>(calloc(1, ctx->mariko_oem_header.bl_size));
    ...
    ctx->pk11 = static_cast<pk11_t *>(calloc(1, ctx->pk11_size));
    if (ctx->pk11 == NULL) {
...
void pk11_save(pk11_ctx_t *ctx) {
    ...
    if (dirpath != NULL && dirpath->valid == VALIDITY_VALID && ctx->is_decrypted) {
        ...
        if (ctx->is_mariko) {
            char *decrypted_bin = static_cast<char *>(malloc(sizeof(ctx->mariko_oem_header) + ctx->mariko_oem_header.bl_size));
            ...
            char *decrypted_bin = static_cast<char *>(malloc(sizeof(ctx->stage1) + ctx->pk11_size));
            if (decrypted_bin == NULL) {
                fprintf(stderr, "Failed to allocate buffer!\n");
                exit(EXIT_FAILURE);
            }
        ...
        if (ctx->is_mariko) {
            ...
            unsigned char *wb_dec = static_cast<unsigned char *>(malloc(wb_size));
            if (wb_dec == NULL) {
...
void pk21_process(pk21_ctx_t *ctx) {
    ...
    ctx->sections = static_cast<unsigned char *>(malloc(ctx->package_size));
    ...
    if (ctx->header.section_sizes[1] > 0) {
        ...
        ctx->ini1_ctx.header = static_cast<ini1_header_t *>(calloc(1, sizeof(ini1_header_t)));
...
void pk21_save(pk21_ctx_t *ctx) {
    ...
    if (dirpath != NULL && dirpath->valid == VALIDITY_VALID) {
        ...
        /* Save Decrypted.bin */
        printf("Saving decrypted binary to %s/Decrypted.bin\n", dirpath->char_path);
        char *decrypted_bin = static_cast<char *>(malloc(ctx->package_size));
        if (decrypted_bin == NULL) {
            fprintf(stderr, "Failed to allocate buffer!\n");
            exit(EXIT_FAILURE);
        }
